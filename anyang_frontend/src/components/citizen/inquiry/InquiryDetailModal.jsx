...
export default function InquiryDetailModal({ inquiry, zIndexClass, onClose, onUpdate, onDelete }) {
  ...
  return (
    <Modal
      open
      onClose={onClose}
      // 편집 중/저장 중에는 배경 클릭·X로 닫히지 않게 해서 작성 중인 내용이 사라지지 않게 한다 (취소/저장 버튼으로만 종료)
      closeDisabled={editing || saving}
      zIndexClass={zIndexClass}
      widthClass="max-w-[640px]"
      className="flex max-h-[min(840px,calc(100vh-48px))] flex-col overflow-hidden text-left"
    >
      <div className="flex items-start justify-between gap-3 border-b border-slate-200 bg-white p-5 pr-14">
